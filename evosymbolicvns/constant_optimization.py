...
class GPEngineWithVNS(GPEngineWithLocalSearch):
    """
    Extiende GPEngineWithLocalSearch añadiendo Variable Neighborhood Search.

    Estrategia combinada en cada generación donde se activa:
      1. optimize_population (scipy)  — afina constantes de los top_k
      2. vns_population (VNS)         — escapa de mínimos locales
                                        estructurales en los top_k

    El orden importa: primero se ajustan constantes (barato y efectivo),
    luego VNS intenta mejorar la estructura si scipy no fue suficiente.

    Parámetros adicionales respecto a GPEngineWithLocalSearch
    ---------------------------------------------------------
    vns_every   : int   — cada cuántas generaciones aplicar VNS
    vns_top_k   : int   — cuántos individuos reciben VNS
    vns_k_max   : int   — número máximo de vecindades (1..k_max)
    vns_iter    : int   — iteraciones del bucle VNS por individuo
    vns_ls_maxiter : int — iteraciones de scipy dentro de cada paso VNS
    """
    ...
    def __init__(
        ...
        *args,
        ...
        vns_k_max:      int = 4,
        vns_iter:       int = 5,
        vns_ls_maxiter: int = 50,
        **kwargs,
    ):
        super().__init__(*args, **kwargs)
        self.vns_every       = vns_every
        self.vns_top_k       = vns_top_k
        self.vns_k_max       = vns_k_max
        self.vns_iter        = vns_iter
        self.vns_ls_maxiter  = vns_ls_maxiter
        self.n_vns_improved_ = 0   # contador acumulado de mejoras VNS

    def run(self, X: np.ndarray, y: np.ndarray, callback=None) -> "GPEngineWithVNS":
        """
        Bucle evolutivo con scipy periódico + VNS periódico.

        Orden dentro de cada generación que activa búsqueda:
          evaluar → scipy (local_search_every) → VNS (vns_every)
                  → actualizar mejor → evolucionar
        """
        from vns import vns_population
        ...
        if self.verbose:
            print(
                f"GPEngineWithVNS: poblacion={self.population_size}  "
                f"scipy cada {self.local_search_every} gens (top_{self.local_search_top_k})  "
                f"VNS cada {self.vns_every} gens (top_{self.vns_top_k}, k_max={self.vns_k_max})"
            )
            print("-" * 65)
        ...
        for gen in range(self.generations + 1):
            ...
            # ── Evaluar fitness ──────────────────────────────────────────
            self.fitnesses_ = evaluate_population(
                self.population_, X, y, self.evaluator
            )

            # ── Paso 1: búsqueda local de constantes (scipy) ─────────────
            apply_ls = (
                self.local_search_every > 0
                and gen % self.local_search_every == 0
                and gen > 0
            ...
            if apply_ls:
                self.population_, self.fitnesses_, n_ls = optimize_population(
                    self.population_, X, y, self.evaluator, self.fitnesses_,
                    top_k=self.local_search_top_k,
                    method=self.local_search_method,
                    maxiter=self.local_search_maxiter,
                )
                self.n_local_search_improved_ += n_ls

            # ── Paso 2: VNS sobre los mejores ────────────────────────────
            apply_vns = (
                self.vns_every > 0
                and gen % self.vns_every == 0
                and gen > 0
            )
            if apply_vns:
                self.population_, self.fitnesses_, n_vns = vns_population(
                    population = self.population_,
                    fitnesses  = self.fitnesses_,
                    X          = X,
                    y_true     = y,
                    evaluator  = self.evaluator,
                    generator  = self.generator,
                    top_k      = self.vns_top_k,
                    k_max      = self.vns_k_max,
                    vns_iter   = self.vns_iter,
                    ls_method  = self.local_search_method,
                    ls_maxiter = self.vns_ls_maxiter,
                )
                self.n_vns_improved_ += n_vns

            # ── Actualizar mejor global ───────────────────────────────────
            gen_best_tree, gen_best_fit = best_individual(
                self.population_, self.fitnesses_
            )
            if gen_best_fit < self.best_fitness_:
                self.best_fitness_ = gen_best_fit
                self.best_tree_    = gen_best_tree.clone()
            ...
            if self.verbose:
                self._print_progress(stats)
                if apply_ls and self.n_local_search_improved_ > 0:
                    print(f"          [scipy] {n_ls} mejoras en esta generacion")
                if apply_vns:
                    print(f"          [VNS  ] {n_vns} mejoras en esta generacion  "
                          f"(total acum: {self.n_vns_improved_})")
            ...
            # ── Evolucionar ───────────────────────────────────────────────
            self.population_ = self.gen_operators.evolve(
                self.population_, self.fitnesses_
            )
        ...
        if self.verbose:
            ...
            print("-" * 65)
            print(f"Finalizado ({self.stopped_reason_}) en {elapsed:.2f}s")
            print(f"Mejoras scipy : {self.n_local_search_improved_}")
            print(f"Mejoras VNS   : {self.n_vns_improved_}")
            print(f"Mejor expresion : {self.best_tree_.to_string()}")
            print(f"Mejor fitness   : {self.best_fitness_:.6f}")

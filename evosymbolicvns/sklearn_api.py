...
from constant_optimization import GPEngineWithLocalSearch, GPEngineWithVNS
...
class SymbolicRegressor(BaseEstimator, RegressorMixin):
    ...
    use_constant_optimization : bool
        Activa búsqueda local de constantes con scipy.
    local_search_every : int
        Cada cuántas generaciones aplicar búsqueda local scipy.
    local_search_top_k : int
        Cuántos individuos optimizar con scipy en cada paso.
    local_search_method : str
        Método de scipy.optimize ('Nelder-Mead' recomendado).
    use_vns : bool
        Activa Variable Neighborhood Search. Requiere
        use_constant_optimization=True para el paso local interno.
    vns_every : int
        Cada cuántas generaciones aplicar VNS.
    vns_top_k : int
        Cuántos individuos reciben VNS en cada paso.
    vns_k_max : int
        Número máximo de vecindades VNS (1=mínima .. 4=máxima).
    vns_iter : int
        Iteraciones del bucle VNS por individuo.
    vns_ls_maxiter : int
        Iteraciones de scipy dentro de cada paso VNS.
    random_state : int
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> "SymbolicRegressor":
        ...
        # Selección del motor según configuración:
        #   use_vns=True                  → GPEngineWithVNS  (VNS + scipy)
        #   use_constant_optimization=True → GPEngineWithLocalSearch (solo scipy)
        #   ninguno                        → GPEngine (GP puro)
        if self.use_vns:
            EngineClass = GPEngineWithVNS
        elif self.use_constant_optimization:
            EngineClass = GPEngineWithLocalSearch
        else:
            EngineClass = GPEngine
        ...
        if self.use_vns or self.use_constant_optimization:
        ...
        if self.use_vns:
            engine_kwargs.update(
                vns_every=self.vns_every,
                vns_top_k=self.vns_top_k,
                vns_k_max=self.vns_k_max,
                vns_iter=self.vns_iter,
                vns_ls_maxiter=self.vns_ls_maxiter,
            )
